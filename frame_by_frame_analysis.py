...
def display_result(image, auto_gui):
    ...
    if recognition_result.gestures:
        ...
        annotated_image = image.copy()  # Use image instead of frame
        ...
        # for each landmark, add its index next to it
        for i, landmark in enumerate(hand_landmarks):
            cv2.putText(annotated_image, f"{i}", (int(landmark.x * annotated_image.shape[1]), int(landmark.y * annotated_image.shape[0])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
